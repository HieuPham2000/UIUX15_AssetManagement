...
class ManageMaintenanceAsset extends FormBase {
  constructor() {
    super();
    let me = this;
    me.fileName = "maintenance-asset";
    me.myInit();
  ...
  initProperty() {
    ...
    me.data = manageMaintainanceAssetData;
  ...
  deleteAction(dataRow) {
    ...
    if(dataRow) {
      ...
      showPopupDanger(Constants.PopupHeaderDeleteRecord, `<span>Bạn có chắc chắn muốn xóa <b>Phiếu ${dataRow.code}: Đề nghị mua sắm tài sản ${dataRow.assetName}</b> hay không?</span>`, deleteFunc.bind(me, dataRow.id));
    }
  ...
  editAction(dataRow) {
    let me = this;
    me.$dialog.find(".text-header").html("Chỉnh sửa thông tin đề nghị mua sắm tài sản");
    me.$dialog.show();
...
var oFormBase = new ManageMaintenanceAsset();
var app = this;

function cellStyleStatus(value, row, index) {
  switch(value) {
    case "Đã phê duyệt":
      return {
        classes: 'bg-green'
