...
class ManageAssetUseRequest extends FormBase {
  ...
  initProperty() {
    ...
    me.data = manageAssetUseRequestData;
  ...
  deleteAction(dataRow) {
    ...
    if(dataRow) {
      ...
      showPopupDanger(Constants.PopupHeaderDeleteRecord, `<span>Bạn có chắc chắn muốn xóa <b>Phiếu ${dataRow.code}: Đăng ký sử dụng tài sản ${dataRow.assetCode} - ${dataRow.assetName}</b> hay không?</span>`, deleteFunc.bind(me, dataRow.id));
    }
  ...
  editAction(dataRow) {
    let me = this;
    me.$dialog.find(".text-header").html("Chỉnh sửa thông tin đăng ký sử dụng tài sản");
    me.$dialog.show();
